...
async function getUser() {
  try {
    ...
    users.map(function (user) {
      ...
      row.appendChild(td);
      td = document.createElement('td');
      td.textContent = user.age; // 사용자 나이을 셀에 추가
      row.appendChild(td);
      td = document.createElement('td');
      td.textContent = user.married ? '기혼' : '미혼'; // 사용자 결혼 상태를 셀에 추가
      row.appendChild(td);
  ...
  try {
    ...
    comments.map(function (comment) {
      ...
      edit.addEventListener('click', async () => {
        ...
        if (!newComment) {
          return alert('내용을 반드시 입력하셔야 함니다'); // 새로운 댓글 내용이 없는 경우 경고를 표시하고 함수를 종료
        }
        try { 
...
document.getElementById('comment-form').addEventListener('submit', async (e) => {
  ...
  try {
    await axios.post('/comments', { id, comment }); // 서버에 새 댓글 등록 요청
